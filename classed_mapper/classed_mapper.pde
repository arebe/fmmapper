...
void draw() {
  ...
  if (points.size()<2 || mode == 1) maper();
    ...
    for (int i = points.size()-1; i > 1; i--) {
      int j = i - 1;
      if (points.get(i).pos.z!=255) {
        x1 = int(points.get(j).pos.x);
        y1 = int(points.get(j).pos.y);
        x2 = int(points.get(i).pos.x);
        y2 = int(points.get(i).pos.y);  
        points.get(i).step(singles, speed);
        d = points.get(i).d;
...
void filler() {
  for (int i = points.size()-1; i > 1; i--) {
    int ha = int(points.get(i).pos.z);
    noStroke();
    fill(0, 0, 100);
    beginShape();
    while (points.get(i).pos.z==ha) {
      vertex(points.get(i).pos.x, points.get(i).pos.y);
      if (i>0)i--;
      else break;
    }
    endShape(CLOSE);
...
void drawLines() {
  ...
  if (points.size()>2) {
    for (int i = points.size()-1; i > 1; i--) {
      int j = i - 1;
      int x1 = int(points.get(j).pos.x);
      int y1 = int(points.get(j).pos.y);
      int x2 = int(points.get(i).pos.x);
      int y2 = int(points.get(i).pos.y);
      col = i-(floor(i/100))*100;
      if (points.get(i).pos.z!=255) stroke(0, 0, 255);
      else stroke(0);
      strokeWeight(2);
      line(x1, y1, x2, y2);
      if (d > 1) d=0;
...
void placePoint() {
  points.add(new MPoint(px, py, index));
}

void blackPoint() {
  //removePoint();
  points.add(new MPoint(px, py, 255));
  index++;
}
...
void savePoints() {
  if (points.size()>2) {
    ...
    for (int i = 1; i < points.size(); i++) {
      ...
      newChild.setFloat("x", points.get(i).pos.x);
      newChild.setFloat("y", points.get(i).pos.y);
      newChild.setFloat("z", points.get(i).pos.z);
